...
from .tool import calculate_char_coverage_set
from .db.repo import UserQnARepo, MatchRepo
...
class MrfzcclCore:
    def __init__(self, config: AstrBotConfig, plugin_dir: Path):
        ...
        if not os.path.isabs(self.storage_dir):
            self.storage_dir = os.path.join(str(self.plugin_dir), self.storage_dir)
        ...
        # 构建临时图片路径
        self.img_tmp_path = Path(os.path.join(self.storage_dir, "tmp"))
...
━━━━━━━━━━━━━━""")
    ...
    def _get_absolute_path(self, path: str) -> str:
        ...
        if not os.path.isabs(path):
            path = os.path.join(str(self.plugin_dir), path)
